...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
            self, 
            dataset_path,
            pose_condition_file_cmlp,
            pose_condition_file=None,
            keyfilter=None,
            bgcolor=None,
            src_type="zju_mocap", pose_step=1,
            **_):
        ...
        if pose_condition_file_cmlp is not None:
            ...
            self.pose_condition_list_cmlp = frameid2pose_condition[::self.pose_step]
        else:
        ...
        if pose_condition_file is not None:
            ...
            self.pose_condition_list = frameid2pose_condition[::self.pose_step]
            assert len(self.pose_condition_list) ==  len(self.pose_condition_list_cmlp)
        else:
            self.pose_condition_list = None

        self.total_frames = 4*len(self.pose_condition_list_cmlp)#cfg.render_frames
    ...
    def __getitem__(self, idx):
        ...
        results['pose_condition_cmlp'] = self.pose_condition_list_cmlp[pose_idx]
        if self.pose_condition_list is not None:
            results['pose_condition'] = self.pose_condition_list[pose_idx]
        results['img_name'] = f'angle{view_idx}_pose{pose_idx*self.pose_step}'
        return results
